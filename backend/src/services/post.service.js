...
async function buildDraftFromTopic(topic, options = {}) {
  ...
  const seriesInfo =
    options.seriesInfo && typeof options.seriesInfo === "object"
      ? options.seriesInfo
      : null;
  const inferManualPostIntent =
    typeof options.inferManualPostIntent === "function"
      ? options.inferManualPostIntent
      : () => "insight";
...
async function runManualPostFlow(ctx, topic, options = {}) {
  ...
  const statusMsg = await ctx.reply(
    `[1/5] Đang research thông tin mới nhất về: ${normalizedTopic}...`,
  );
